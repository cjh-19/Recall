...
namespace Team1
{
    public partial class Form1 : Form
    {
        ...
        {
            if (g_rqname.CompareTo(e.sRQName) == 0) // 요청한 요청명과 API로부터 응답받은 요청명이 동일한지 비교합니다
            {
                ; // 다음으로 진행
            }
            else // 같지 않다면
            {
                ...
                write_err_log("응답받은 TR : [" + e.sRQName + "]\n", 0); //어떤 요청을 보냈고 어떤 요청에 대한 응답을 받았는지 로그에 기록한다..
            ...
            if (e.sRQName == "호가조회")
            {
                int cnt = 0;
                int ii = 0;
                int l_buy_hoga = 0; //호가조회' 요청에 대한 결과 처리를 위한 변수를 초기화

                cnt = axKHOpenAPI1.GetRepeatCnt(e.sTrCode, e.sRQName); //응답 데이터의 반복 횟수를 가져옴

                for (ii = 0; ii < cnt; ii++)
                {
                    l_buy_hoga = int.Parse(axKHOpenAPI1.CommGetData(e.sTrCode, "", e.sRQName, ii, "매수최우선호가").Trim());
                    l_buy_hoga = System.Math.Abs(l_buy_hoga); //모든 응답 데이터를 순회하면서 "매수최우선호가" 데이터를 추출하고, 절대값을 취한 후 저장.
                }
            ...
            if (e.sRQName == "증거금세부내역조회요청")
            {
                ...
                axKHOpenAPI1.DisconnectRealData(e.sScrNo); //TR요청 끝났으니 실시간연결 종료함.
                g_flag_1 = 1; //'호가조회' 요청이 완료되었음을 알리기 위해 g_flag_1 플래그를 1로 설정.
            }
            ...
            if (e.sRQName == "계좌평가현황요청") //응답받은 요청명이 '계좌평가현황요청'이라면
            {
                ...
                for (ii = 0; ii < repeat_cnt; ii++) //보유 종목 수 만큼의 반복문을 시작한다. 각 반복에서 사용자 ID, 종목 코드, 보유 주식 수, 매입 가격, 보유 금액을 초기화함.
                {
                    user_id = "";
                    jongmok_cd = "";
                    own_stock_cnt = 0;
                    ...
                    //각 보유 종목에 대한 정보를 추출하여 해당 변수에 저장한다. 이 정보에는 사용자 ID, 종목 코드, 종목명, 보유 수량, 평균 단가, 매입 금액이 포함된다.
                    user_id = g_user_id;
                ...
                write_msg_log("TB_ACCNT_INFO 테이블 설정 완료\n", 0); //모든 보유 종목에 대한 정보를 수집하고 저장한 후, 이 작업이 완료되었음을 로그에 출력한다. 그런 다음, 이 요청과 관련된 데이터 스트림을 해제/
                axKHOpenAPI1.DisconnectRealData(e.sScrNo);

                if (e.sPrevNext.Length == 0) //페이징 처리를 위한 코드 => OpenAPI의 TR 요청은 한 번에 제한된 수의 데이터만 반환하므로, 여러 페이지에 걸친 데이터를 가져올 때는 '계속 요청'이 필요할 수 있다, 그해서 '0'일경우 더이상 다음 페이지가 없다는 뜻으로 0으로 설ㅈㅇ해주게 된다.
                {
                    g_is_next = 0;
                }
                else
        ...
        public void set_tb_accnt() //계좌 테이블 세팅 메서드
        {
            ...
            for (; ; )
            {
                ...
                for (; ; )
                {
                    ...
                    else //아직 요청에 대한 응답이 오지 않을 경우 //로그에 메시지를 출력하고, 1초를 대기합니다. 
                    {
        ...
        public void set_tb_accnt_info() //계좌정보 테이블 설정 , 이 메서드는 TB_ACCnT_INFO 테이블을 삭제한뒤 Open API 의 setInputValue 함수를 이용하여 입력값을 설정하고 CommRqData 함수를 호출햐여 계좌정보를 요청한다.
        {
            OracleCommand cmd;
            OracleConnection conn;
            String sql;
            ...
            sql = @"delete from tb_accnt_info where ref_dt = to_char(sysdate, 'yyyymmdd') and user_id = " + "'" + g_user_id + "'"; //tb_accnt_info 테이블에서 해당 사용자의 계좌 정보를 현재 날짜 기준으로 삭제
            ...
            for (; ; ) //계좌 평가 현황 정보 요청 및 수신 루프 시작
            {
                ...
                for (; ; )
                {
                    ...
                    for (; ; )
                    {
                        if (g_flag_2 == 1) //데이터 수신이 완료되면 일정 시간을 지연시키고, 데이터 스트림을 끊어서 이 루프를 종료하고 다음 페이지의 데이터를 요청하도록 설정한다.
                        {
                            delay(1000);
                            axKHOpenAPI1.DisconnectRealData(l_scr_no);
                            l_for_flag = 1;
        ...
        public void merge_tb_accnt(int g_ord_amt_possible) //계좌정보 테이블 세팅 메서드
        {
            ...
            if (conn != null) // 연결 성공한경우 다음 단계로 넘어감
            {
                cmd = new OracleCommand();
...
" and ref_dt = to_char(sysdate, 'yyyymmdd') ";
        ...
        private string getResults()            // 뉴스 기사 업데이트에 쓰이는 결과값 받아오기 함수임
        {
            ...
            string query = string.Format("?query={0}&display={1}sort={2}", keyword, display, sort); //naver api 설정 홈페이지에있는 불러오는 값들
